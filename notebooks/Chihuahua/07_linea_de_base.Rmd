...
col_infraestructura <- colnames(infraestructura)
```
...
educacion <- full_join(db_2_1_desempeño_cct, db_2_2_agentes_recursos_cct,
                       by=c("cct","cve_ent","cve_mun","cve_loc")) %>% 
...
col_educacion <- colnames(educacion)
```
...
col_intervencion <- colnames(intervencion)
```
...
linea_de_base <- full_join(infraestructura, educacion, by=c("cve_ent","cve_mun","cct", "cve_loc")) %>% 
  ...
  select(colnames(infraestructura),
